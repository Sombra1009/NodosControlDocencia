	...
	for (int8_t blockOffset = no_of_blocks - 1; blockOffset >= 0; blockOffset--) {
		...
		if (group != 3 && (g[group] == 1 || g[group] == 6)) { // Not a sector trailer, a value block
			uint32_t value = ((uint32_t)(buffer[3])<<24) | ((uint32_t)(buffer[2])<<16) | ((uint32_t)(buffer[1])<<8) | (uint32_t)(buffer[0]);
				printf(" Value=0x"); printf("%x",(unsigned int)value);
				printf(" Adr=0x"); printf("%x",(unsigned int)buffer[12]);
...
uint8_t i;
	...
	for (uint8_t page = 0; page < 32; page +=4) { // Read returns data for 4 pages at a time.
		// Read pages
		byteCount = sizeof(buffer);
